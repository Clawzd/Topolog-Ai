...
function buildSystemPrompt(mapState) {
  ...
  return [
    ...
    'If the prompt names a specific subtype (e.g., "10 laptops", "tablets only", "all desktops", "15 desktop workstations"), respect it exactly. For generic "workstations" or "computers" without qualifiers, pick ONE type that fits the scenario (e.g., a small modern office = laptops; a classroom = pcs). Do NOT mix in phones, printers, TVs, cameras, or tablets just because the office has employees.',
    'CRITICAL — do not silently double the endpoint count: if the prompt names ONE workstation subtype (e.g., "15 desktop workstations" → only `pc`, "10 laptops" → only `laptop`, "12 PCs" → only `pc`), emit ONLY that subtype. Do NOT add `laptop` nodes alongside requested `pc` nodes (or vice versa) to "round out" a room. Empty rooms stay empty.',
    'When the prompt assigns a count to a SPECIFIC room (e.g., "Open Office: 15 desktop workstations"), put exactly that many of that type in that room and zero of that type in other rooms unless those rooms also list a count. Do not redistribute the count across rooms.',
    ...
    '- EVERY endpoint device — workstation, laptop, phone, printer, camera, tablet, IoT, smart TV, NAS, server, AP — MUST sit inside one of the rooms. Its (x,y) center must lie within a room rectangle. No floating endpoints, no APs outside their floor.',
    '- A room must contain ALL its devices with at least 96px padding on each side AND fit them in a roughly square grid (do NOT stack endpoints in a single tall column).',
    '- Make rooms generously sized: minimum 480px wide and 320px tall. Width should grow with device count: ~480px for 1–3 devices, ~640px for 4–6, ~800px for 7–9, ~1000px for 10+. Height grows similarly. Aim for ceil(sqrt(N)) columns of devices and at least 120px per device-cell.',
    '- Rooms must NOT overlap each other. Leave at least 64px gap between room rectangles.',
    '- ARRANGE ROOMS HORIZONTALLY across the canvas — multiple rooms should sit SIDE BY SIDE in rows, not stacked in one tall column. For 3+ rooms, lay them out across the canvas width (e.g. row 1: room A | room B | room C). The canvas is wide; do NOT compress everything into a vertical strip.',
    ...
    '- Group related endpoints together: a VoIP phone goes in the SAME room as the workstation it serves; a printer goes in the room of the team that uses it; an AP goes in the room (or floor) it covers. Do NOT leave phones, printers, cameras, or APs dangling outside any room.',
    '- Every AP MUST have at least one link to its floor/zone switch (or the core switch). Never emit an AP with zero links.',
