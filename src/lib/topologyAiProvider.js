...
function buildSystemPrompt(mapState) {
  ...
  return [
    ...
    '- EVERY endpoint device — workstation, laptop, phone, printer, camera, tablet, IoT, smart TV, NAS, server — MUST sit inside one of the rooms. Its (x,y) center must lie within a room rectangle. No floating endpoints.',
    '- A room must contain ALL its devices with at least 72px padding on each side AND fit them in a roughly square grid (do NOT stack endpoints in a single tall column).',
    '- Make rooms generously sized: minimum 360px wide and 240px tall. Width should grow with device count: ~360px for 1–3 devices, ~520px for 4–6, ~680px for 7–9, ~840px for 10+. Height grows similarly. Aim for ceil(sqrt(N)) columns of devices.',
    ...
    '- Group related endpoints together: a VoIP phone goes in the SAME room as the workstation it serves; a printer goes in the room of the team that uses it. Do NOT leave phones, printers, or cameras dangling outside any room.',
