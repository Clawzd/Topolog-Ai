...
export default function NetworkInsightsPanel({
  ...
  return (
    <aside
      className={`absolute bottom-3 left-3 z-20 overflow-hidden rounded-xl border border-border/60 bg-card/95 shadow-xl shadow-black/30 backdrop-blur-md flex flex-col transition-[max-height,width] duration-200 ${
        panelExpanded
          ? 'w-[min(360px,calc(100vw-11rem))] max-h-[min(400px,min(46dvh,52vh))]'
          : 'w-[min(300px,calc(100vw-11rem))]'
      }`}
    >
      <div className="flex items-center justify-between gap-2 border-b border-border/60 px-3 py-2 flex-shrink-0">
        <div className="flex min-w-0 items-center gap-2">
          <Network className="h-4 w-4 flex-shrink-0 text-primary" />
          <div className="min-w-0">
            <div className="truncate text-xs font-semibold text-foreground">Network Intelligence</div>
            {!panelExpanded && (
              <div className="truncate text-[9px] text-muted-foreground">
                Score {displayOverall} · {mergedFindings.length} finding{mergedFindings.length === 1 ? '' : 's'} · expand for details
              </div>
            )}
          </div>
        </div>
        <div className="flex flex-shrink-0 items-center gap-0.5">
          <button
            ...
            title={panelExpanded ? 'Collapse panel' : 'Expand details'}
          >
